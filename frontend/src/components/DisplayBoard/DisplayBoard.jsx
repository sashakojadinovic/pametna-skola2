...
import api from "../../api/axiosInstance"; // ✅ Dodato
  ...
  return (
    <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 3, py: 1.5, minHeight: 72 }}>
      ...
      <Stack direction="row" alignItems="center" spacing={1.5}>
        ...
        <Tooltip title={audioReady ? "Звук укључен" : "Звук је блокиран — омогући"}>
...
export default function DisplayBoard() {
  ...
  // ✅ API fetch za aktivna obaveštenja
  ...
  return (
    <Box sx={{ height: "100dvh", display: "flex", flexDirection: "column", bgcolor: "background.default", color: "text.primary", overflow: "hidden" }}>
      ...
      <Box sx={{ flex: 1, minHeight: 0, px: 3, pb: 3, display: "flex", alignItems: "stretch", justifyContent: "center" }}>
        {isHoliday ? (
          <Typography sx={{ fontSize: "clamp(40px, 8vw, 120px)", fontWeight: 800, textAlign: "center" }}>Данас је нерадни дан</Typography>
        ) : (
          <Stack spacing={2} alignItems="center" sx={{ flex: 1, minHeight: 0, width: "100%" }}>
            ...
            } color={current?.type === "ЧАС" ? "error" : "success"} sx={{ fontSize: "clamp(14px, 2vw, 24px)", px: 2.5, py: 2, borderRadius: 3 }} />
            <MiddleCircle nowTick={nowTick} current={current} />
            <Typography sx={{ fontSize: "clamp(16px, 2vw, 24px)" }}>
              Следеће звоно у {nextTs ? formatClock(nextTs) : "—"} {nextBell?.label && (<Box component="span" sx={{ textTransform: "lowercase", ml: 1 }}>({nextBell.label})</Box>)}
            </Typography>
            {!current && (
              <>
